...
- [x] Profile raster-heavy styles against existing backends. Local macOS Debug render-test timing on 2026-05-02: Skia 3.53s with 41 passed, 1 ignored, and 1 failed; Metal 3.19s with 42 passed and 1 ignored. The original Skia failure in this subset was `image/pitched`; it now passes after raster near-plane clipping.
...
- [x] Add per-test workpads for active Skia render failures. `SKIA_RENDER_FAILURES/` tracks the current active failure set, inspection notes, aggregate patterns, and archive workflow.
- [ ] Reach parity target for initial 2D layer set. Target: every render test is either already ignored on main, passing with Skia, or failing with a narrowly documented deferral reason. Fill extrusion depth-buffer semantics are the expected acceptable deferral class; other failures are fix-required unless a similarly strong blocker is identified. Current full-suite Skia baseline on 2026-05-02 after circle sort-key and line premultiplication fixes: 1272 passed, 12 ignored passed, 79 ignored, and 39 failed.
