...
if(MLN_LINUX_STATIC_DEPS)
    message(STATUS "Using vendored static dependencies (MLN_LINUX_STATIC_DEPS=ON)")
...
if(MLN_LINUX_STATIC_DEPS AND "${ARMERGE}" STREQUAL "ARMERGE-NOTFOUND")
    message(FATAL_ERROR
        "MLN_LINUX_STATIC_DEPS requires armerge for static library amalgamation.\n"
...
if(NOT ICUUC_FOUND OR "${ICUUC_VERSION}" VERSION_LESS 62.0 OR MLN_USE_BUILTIN_ICU)
    message(STATUS "ICU not found, too old or MLN_USE_BUILTIN_ICU requested, using builtin.")
...
# Bundle libraries into amalgamated static archive
if(NOT "${ARMERGE}" STREQUAL "ARMERGE-NOTFOUND")
    message(STATUS "Found armerge: ${ARMERGE}")

    if(MLN_LINUX_STATIC_DEPS)
        # Vendored path: all deps are built with -fPIC, safe to amalgamate
        add_custom_command(
            TARGET mbgl-core
            POST_BUILD
            # No --keep-symbols: all vendored symbols stay global inside the
            # amalgamated .a so the final .so linker resolves them internally
            # rather than falling back to system shared libs.
            ...
            # so downstream targets (maplibre-jni) link against the full bundle
        ...
        # other native libraries in the same process that may use the same deps
        target_link_options(mbgl-core PUBLIC -Wl,--exclude-libs,ALL)
    elseif(NOT MLN_USE_BUILTIN_ICU)
        # System path: use system static libraries (may lack -fPIC)
